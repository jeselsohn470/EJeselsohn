...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    private class Node {
        ...
        private Entry[] entries = new BTreeImpl.Entry[BTreeImpl.MAX]; // the array of children
        private Node next;
    ...
    public Value get(Key k) {
        ...
        if (entry != null) {
            if (entry.val ==null){
                try {
                    entry.val = this.pm.deserialize((Key) entry.key);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    public Value put(Key k, Value v) {
        ...
        if(alreadyThere != null)
        {
            if (alreadyThere.val ==null) {
                try {
                    alreadyThere.val = this.pm.deserialize((Key) alreadyThere.key);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            Value oldV = (Value) alreadyThere.val;
